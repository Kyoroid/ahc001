...
def read_from_file(path: Path):
    with path.open(mode="r") as f:
        ...
        XYR = []
        for i in range(N):
            XYR.append(list(map(int, f.readline().strip().split())))
    return N, XYR
...
def step1(N, XYR):
    ...
    # 1.2. search from largest
    # order = argsort([-r for x, y, r in XYR])
    # for i in order:
    #     random_dilate(N, XYR, ads, i, T=30, U=30)
    voronoi_dilate(N, XYR, ads, M=10000)
    return ads


def step2(N, XYR, ads, n, a_th):
    annealing_v1(N, XYR, ads, n=n, a_th=a_th)
