...
def left_shift(i, x, y, r, ad, ads):
    ...
    while abs(ok - ng) > 1:
        ...
        if point_in_rect(x, y, new_ad) and check_adspace(new_ad, ads):
            ...
            ng = mid
    new_ad = (i, left - ok, top, right - ok, bottom)
    if point_in_rect(x, y, new_ad) and check_adspace(new_ad, ads):
        return ok
    return -1


def top_shift(i, x, y, r, ad, ads):
    _, left, top, right, bottom = ad
    ok, ng = 0, bottom
    while abs(ok - ng) > 1:
        mid = (ok + ng) // 2
        ...
        if point_in_rect(x, y, new_ad) and check_adspace(new_ad, ads):
            ...
            ng = mid
    new_ad = (i, left, top - ok, right, bottom - ok)
    if point_in_rect(x, y, new_ad) and check_adspace(new_ad, ads):
        return ok
    return -1


def offset_adspace(i, x, y, r, ads, T = 20):
    # 1. dilate and shift
    ad = ads[i]
    update = False
    for t in range(T):
        ...
        best_left, best_top, best_width, best_height = 0, 0, 1, 1
        for width in div[::-1]:
            height = int(ar / width)
            new_ad = (i, x, y, x + width, y + height)
            left = left_shift(i, x, y, r, new_ad, ads)
            new_ad = (i, x - left, y, x - left + width, y + height)
            top = top_shift(i, x, y, r, new_ad, ads)
            if left >= 0 and top >= 0 and best_left + best_top < left + top:
                best_left, best_top, best_width, best_height = left, top, width, height
                update = True
        if update:
            break
    return (i, x - best_left, y - best_top, x - best_left + best_width, y - best_top + best_height)
...
def solve(N, IXYR):
    ...
    for i, x, y, r in IXYR:
        ads[i] = offset_adspace(i, x, y, r, ads)
